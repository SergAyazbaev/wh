...
class Past_inventory_cs_Controller extends Controller
{
    ...
    public function actionIndex()
    {
        ...
        if (!isset($para['print'])) {
            $session['past_inventory_cs_'] = $para; // Это то самый секрет!!!
        } else {
            $para = $session['past_inventory_cs_'];  //  тут его получаем
        }
        ////////////
        //ddd($para['post_cs_past_inventory']['wh_destination']);

        $searchModel = new post_cs_past_inventory();
        $dataProvider = $searchModel->search($para);

        $filter_spisok = Sklad_cs_past_inventory::ArrayUniq_WhElements_Ids();

        if (isset($para['post_cs_past_inventory']['wh_destination'])) {
            $filter_spisok = Sklad_cs_past_inventory::ArrayBy_destination($para['post_cs_past_inventory']['wh_destination']);
            //ddd($filter_spisok);
        }


        $filter_spr = [0 => "Выбрать все"] + Sprwhelement::ArrayNamesWithIds($filter_spisok);
        $filter_gos = [0 => "Выбрать все"] + Sprwhelement::ArrayGosWithIds($filter_spisok);
        $filter_bort = [0 => "Выбрать все"] + Sprwhelement::ArrayBortWithIds($filter_spisok);
...
//        }
        ...
        return $this->render(
            'index', [
                ...
                'filter_spr' => $filter_spr,
                'filter_gos' => $filter_gos,
                'filter_bort' => $filter_bort,
            ]
        );
    ...
    */
    ...
    public function actionUpdate($id)
    {
        ...
        ////////
        $model = Sklad_cs_past_inventory::findModel($id);  // it is =  _id
        ...
        if (isset($para_post['contact-button']) && $para_post['contact-button'] == 'erase_button') {
            ...
            if (is_array($model['array_tk_amort'])) {
                ...
                $stop = (int)$para_post['Sklad_inventory']['erase_array'][0][1] -
                    $start;
            ...
            if (is_array($model['array_tk'])) {
                ...
                $stop = (int)$para_post['Sklad_inventory']['erase_array'][1][1] -
                    $start;
            ...
            if (is_array($model['array_casual'])) {
                ...
                $stop = (int)$para_post['Sklad_inventory']['erase_array'][2][1] -
                    $start;
...
//        ddd($model);
//        ddd($this->summa_itog($model->array_tk_amort));
...
//        return $this->redirect(Yii::$app->request->referrer);
        ...
        return $this->render(
            ...
            [
                'new_doc' => $model,
                'spr_things' => $spr_things,
                'itogo' => $this->summa_itog($model->array_tk_amort),
                'alert_mess' => '',
            ]
        );
...
//        $searchModel = new post_past_inventory();
        ...
        $query = Sklad_cs_past_inventory::find()->all();
...
//            $query->andFilterWhere(['=', 'wh_destination_element', $para['id'] ]);
    ...
    public function actionCreate_new()
    {
        ...
        $max_value = Sklad_cs_past_inventory::find()->max('id');
        $max_value++;
...
//        $para_post = Yii::$app->request->post();
    ...
    public function actionPdf_form()
    {
        ...
        $model = Sklad_cs_past_inventory::findModelDouble($para['id']);
        ...
        ///// BAR-CODE
        //1
        $html_css = $this->getView()->render('/past_inventory_cs_/pdf_form/_form_css.php');

        //ddd($model);
        //2
        $html = $this->getView()->render(
            '/past_inventory_cs_/pdf_form/_form', [
    ...
    public function actionExcel_form_inventory()
    {
        ...
        $model_array_tk = Sklad_cs_past_inventory::sort_array_pk($model['array_tk']);
        //ddd($model_array_tk);
        ////////////////////
        ///// AMORT!!
        $model1 = ArrayHelper::map(
    ...
    {
        Sklad_cs_past_inventory::findModel($id)->delete();
        return $this->redirect(['/past_inventory_cs_/' . $adres_to_return]);
    ...
    public function getTkNames_am($array_tk)
    {
        ...
        if (isset($array_tk) && !empty($array_tk))
            foreach ($array_tk as $key => $item) {
                ...
                $buff[$key]['dt_create_plus'] = (isset($item['dt_create_plus']) ? $item['dt_create_plus'] : '');
                $buff[$key]['dt_create_minus'] = (isset($item['dt_create_minus']) ? $item['dt_create_minus'] : '');

//                'dt_create_plus' => 1575687008
//                'dt_create_minus' => 1576288818
            }
