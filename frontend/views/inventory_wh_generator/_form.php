...
<div id="tz1">
    <div class="pv_motion_create_right_center">
        <div class="tree_land3">
            <?php
            echo "<br><h2> WH. Столбовые ИНВ  </h2>";
            ?>
        </div>
        ...
        <div class="tree_land">
            ...
            echo $form->field($model, 'wh_destination_element')->widget(
                Select2::classname(), [
                'data' => $list_element,
                'options' => ['placeholder' => 'Список автобусов ...'],
                'pluginOptions' => [
                    'allowClear' => true
                ],
            ]);
    ...
    <div class="pv_motion_create_ok_button">
        ...
        <?php
        echo Html::a('Выход', ['/sklad_inventory_wh'], ['class' => 'btn btn-warning']);
        ?>



        <?= Html::submitButton('Создать Инвентаризацию ПЕ для ЦС',
            [
...
///  
$('#sklad_inventory_wh-dt_create').change(function() {    
    $("#sklad_wh_invent-wh_destination").val('');
    $("#sklad_wh_invent-wh_destination_element-container").html('');
...
$('#sklad_wh_invent-wh_destination').change(function() {
    ...
    var  number = $(this).val();    
    var actualdate = $('#sklad_wh_invent-dt_create').val(); // 30.11.2020 23:59:59

    //console.log(actualdate);

    $.ajax( {
		url: '/inventory_wh_generator/list_element_wh',
		data: {
		    id :number,
		...
		//dataType: "json",		
			success: function(res) {		 
		            //$('#sklad_inventory_wh-wh_destination_element').html('');
		            //$('#sklad_inventory_wh-wh_destination_element').html(res);		    
		            $('#sklad_wh_invent-wh_destination_element').html(res);		    
					},
			error: function( res) {
						alert('JS.sklad_wh_invent-wh_destination == '+res );
						console.log(res);
					}
    } );
...
// sklad_inventory_wh-dt_create

//////////////////// destination_element - element
 
$('#sklad_wh_invent-wh_destination_element').change(function() {    
     var  number2 = $('#sklad_wh_invent-wh_destination_element').val();     
     var  text2   = $('#sklad_wh_invent-wh_destination_element>option[value='+number2+']').text();
      $('#sklad_wh_invent-wh_destination_element_name').val(text2);
});
