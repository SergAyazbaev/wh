...
{
    $DateOld = date('d.m.Y',strtotime($para['postpvmotion']['dt_create_start']));
    $TimeOld = date('H:i',strtotime($para['postpvmotion']['dt_time_start']));

}
else
{
    $DateOld = date('d.m.Y',strtotime("Monday last week"));
    $TimeOld = date('H:i',strtotime("Monday last week"));
}

if (isset($para['postpvmotion']['dt_create_stop'] ))
{
    $DateNow = date('d.m.Y',strtotime($para['postpvmotion']['dt_create_stop']));
    $TimeNow = date('H:i',strtotime($para['postpvmotion']['dt_time_stop']));
}
else
{
//    $DateNow = date('d.m.Y',strtotime("saturday last week" ));
...
$DateStart = date('d.m.Y',strtotime("first day of  -1 month " ));
//$TimeStart = date('H:i',strtotime("first day of  -1 month "));
$DateEnd = date('d.m.Y',strtotime("now + 15 day" ));
