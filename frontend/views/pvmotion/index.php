...
if(Yii::$app->request->queryParams['postpvmotion']['pv_id']>0)
    $this->title = 'История перемещений (Инв.№ '.Yii::$app->request->queryParams['postpvmotion']['pv_id'].')';
else
    $this->title = 'История перемещений (для всех устройств)';
    ...
    <p>
        <div class="date_period1">
            <?
                ...
                if( Yii::$app->request->queryParams['postpvmotion']['pv_id']>0 ) {
                    echo Html::a('Запись о перемещении',
...
//            'wh_cred_element',
            ...
            [
                'attribute'=> 'wh_cred_top',
                //'filter'=> frontend\models\Sprwhtop::find()->select(['name'])->indexBy('name')->column(),
                'filter' => \yii\helpers\ArrayHelper::map(frontend\models\Sprwhtop::find()->all(), 'id', 'name'),
                'value'=> 'wh_cred_top_name',
                //'contentOptions' => ['style' => 'max-width: 50px;overflow: auto;'],
            ],
...
$('#date_start').change(function() {   
    ...
    var wh_deb_top_name= $('select[name="postpvmotion[wh_deb_top_name]"]').val();
       ...
       $.ajax({
                ...
                data: {
                    ...
                    postpvmotion:{
                        ...
                        document        :document,
                        wh_deb_top_name     :wh_deb_top_name,
