...
$activeSheet->mergeCells('A3:E7');
$activeSheet->getStyle('A3:E7')->getAlignment()->setWrapText(true); //!!!
...
$activeSheet->mergeCells('A8:E12');
$activeSheet->getStyle('A8:E12')->getAlignment()->setWrapText(true); //!!!
...
$activeSheet->setCellValue('D17', 'К-во');
$activeSheet->setCellValue('E17', '1C-id');

$activeSheet->getRowDimension(17)->setRowHeight(40); /// ВЫСОТА !!!!


$activeSheet->setSharedStyle($sharedStyle1, "A17:E17");
$activeSheet->getStyle('A17:E17')->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
...
foreach ($model['array_tk_amort'] as $key_group => $item) {
    ...
    $activeSheet->setCellValue('E' . ($num), "" . $model_amcc[$item['wh_tk_element']]);

    $num++;
    $num_str++;
}
$num--;
$activeSheet->setSharedStyle($sharedStyle1, "A18:E18");
$activeSheet->setSharedStyle($sharedStyle2, "A18:E" . ($num));
...
foreach ($model_array_tk as $key_group => $item) {
    if ($key_group_fub != (int)$item['wh_tk']) {
        ...
        $activeSheet->setSharedStyle($sharedStyle1, "A" . $num . ":E" . ++$num);
        $activeSheet->mergeCells('A' . --$num . ':E' . ++$num);

        $num++;

        $activeSheet->setSharedStyle($sharedStyle2, "A" . ($num_start) . ":E" . ($num - 3));
        $activeSheet->setSharedStyle($sharedStyle3_num, 'D' . ($num_start) . ':E' . ($num - 3));
    ...
    $activeSheet->setCellValue('C' . ($num), "" . $things[$item['ed_izmer']]);
    $activeSheet->setCellValue('D' . ($num), "" . $item['ed_izmer_num']);
    $activeSheet->setCellValue('E' . ($num), "" . $model_cc[$item['wh_tk_element']]);
...
$activeSheet->setSharedStyle($sharedStyle2, "A" . ($num_start) . ":E" . ($num - 1));
$activeSheet->setSharedStyle($sharedStyle3_num, 'D' . ($num_start) . ':E' . ($num - 1));
...
$filename = 'invent_op_' . date('d.m.Y H-i-s') . '.xls';
