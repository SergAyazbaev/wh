...
//if (isset($_REQUEST['postpvmobile']) && !empty($_REQUEST['postpvmobile']))
//{
//    if (isset($_REQUEST['postpvmobile']['dt_create']))
//        $DateStart=date('d.m.Y', strtotime($_REQUEST['postpvmobile']['dt_create']));
//
//
//    if (isset($_REQUEST['postpvmobile']['dt_create_end']))
//        $DateStop=date('d.m.Y', strtotime($_REQUEST['postpvmobile']['dt_create_end']));
//
//}
//
//
...
//dd( Yii::$app->request->queryParams['postpvmobile']['pv_id'] );

if(Yii::$app->request->queryParams['postpvmobile']['pv_id']>0)
    $this->title = 'Действия по снятию/установке  (Инв.№ '.Yii::$app->request->queryParams['postpvmobile']['pv_id'].')';
else
    $this->title = 'Действия по снятию/установке (для всех устройств)';
    ...
    <p>
        <div class="date_period1">
            <?
                ...
                if( Yii::$app->request->queryParams['postpvmobile']['pv_id']>0 ) {
                    echo Html::a('Новая запись о перемещении',
                        ['create?pv_id='.Yii::$app->request->queryParams['postpvmobile']['pv_id'] ],
...
//                dd($para);
                ...
                if ( isset($para['postpvmobile']) && !empty($para['postpvmobile'])){
                    foreach ($para['postpvmobile'] as $key => $item) {
                ...
                echo  Html::a('Сводная ведомость', ['/pvmobile/printsvod'.$para_str],
                    ['target' => '_blank','class' => 'btn btn-success',
                        'style' => 'margin-right:3px' ]);

                echo  Html::a('Печать  АПП', ['/pvmobile/printapp'.$para_str],
                    ['target' => '_blank','class' => 'btn btn-success',
                        'style' => 'margin-right:3px']);

                echo  Html::a('Печать  АДВ', ['/pvmobile/printadv'.$para_str],
                    ['target' => '_blank','class' => 'btn btn-success',
                        'style' => 'margin-right:3px' ]);
            ?>
...
//            '_id',
            ...
            [
                ...
                'filter' => \yii\helpers\ArrayHelper::map(frontend\models\Typeact::find()->all(), 'id', 'tx'),
...
$('#date_start').change(function() {   
    ...
    var pv_id= $('input[name="postpvmobile[pv_id]"]').val();
    var name= $('input[name="postpvmobile[name]"]').val();
    var type_action_tx= $('select[name="postpvmobile[type_action_tx]"]').val();
    var document= $('input[name="postpvmobile[document]"]').val();
    var wh_top_name= $('select[name="postpvmobile[wh_top_name]"]').val();
    var wh_element = $('input[name="postpvmobile[wh_element]"]').val();
