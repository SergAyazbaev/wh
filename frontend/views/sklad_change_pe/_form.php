			...
			<?php
				...
				echo $form->field( $model, 'wh_dalee_element' )->widget(
					...
					, [
						...
						'data'  => ArrayHelper::map(
							Sprwhelement::find()
							            ->where( [ 'parent_id' => (integer) $model->wh_dalee ] )
							            ->all(), 'id', 'name' ),
						'theme' => Select2::THEME_BOOTSTRAP,
						'size'  => Select2::SMALL,
						//LARGE,
				...
				'columns' => [
					...
					[
						...
						'items'        => function( $data ) {
							...
							//if( !isset( $data[ 'wh_tk_amort' ] ))
							if( !isset( $data[ 'name_wh_tk_amort' ] ))
							{
								return
									ArrayHelper::map(
...
if (event.keyCode==13){
        ...
        data = data.replace(/[a-zA-ZА-Яа-яЁё]/g,'');
