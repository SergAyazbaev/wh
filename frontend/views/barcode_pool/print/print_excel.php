<?php
	...
	$activeSheet->setCellValue( 'A1', 'Справочник. ПУЛ Штрихкодов для проверки' );

	$ii = $dataModels[ 0 ];
	$activeSheet->setCellValue( 'A2', $spr_globam[ (int) $ii[ 'spr_globam_element' ][ 'parent_id' ] ] );
	$activeSheet->setCellValue( 'A3', $spr_globam_element[ (int) $ii[ 'element_id' ] ] );
	...
	$activeSheet->getStyle( 'A3' )->getFont()->setSize( 12 );
	...
	$activeSheet->setCellValue( 'C4', 'ИД ПАРТИИ' );
	$activeSheet->setCellValue( 'D4', 'Наименование' );
	$activeSheet->setCellValue( 'E4', 'Штрихкод' );
	...
	foreach( $dataModels as $ii )
	{
		...
		$activeSheet->setCellValue( 'C' . ( $num + 5 ), $ii[ 'barcode_consignment_id' ] );
		...
		$activeSheet->setCellValue( 'E' . ( $num + 5 ), $ii[ 'bar_code' ] );
