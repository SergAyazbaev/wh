...
$activeSheet->setCellValue('A1', 'Сводная ведомость ');
...
$activeSheet->getRowDimension('A1:F2')->setRowHeight(50);
...
$activeSheet->getStyle('F1')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);
...
$activeSheet->setCellValue('D4', 'ШтрихКод');
$activeSheet->setCellValue('E4', 'Ед.Изм');
$activeSheet->setCellValue('F4', 'Кол.');
...
foreach ($array_PE as $item_PE) {
    ...
    if (isset($item_PE['wh_destination_name']) && !empty($item_PE['wh_destination_name'])) {
        ...
        $activeSheet->mergeCells('D' . ($num + 5) . ':E' . ($num + 5));
...
//            if ((int)$ii['ed_izmer_num'] > 0) {
            ...
            //$activeSheet->setCellValue('D' . ($num + 5), ' ' . $ii['bar_code'], PHPExcel_Cell_DataType::TYPE_STRING);
            $activeSheet->setCellValue('D' . ($num + 5), ' ' . $ii['bar_code']);

            $activeSheet->setCellValue('E' . ($num + 5), $ii['ed_izmer_name']);

            $activeSheet->setCellValue('F' . ($num + 5), (int)$ii['ed_izmer_num'], PHPExcel_Cell_DataType::TYPE_NUMERIC);

            //$activeSheet->setCellValue('G' . ($num + 5), (int)$ii['itog'], PHPExcel_Cell_DataType::TYPE_NUMERIC);
...
//            }
        ...
        $activeSheet->getStyle('D5:D' . ($num + 5))->getNumberFormat()->setFormatCode(PHPExcel_Style_NumberFormat::FORMAT_TEXT);


        $activeSheet->setCellValue('D' . ($num_str_itogo), 'Итого:' . $itogo);
        // Стиль
        $activeSheet->setSharedStyle($sharedStyle1, "A" . ($num_str_itogo) . ":F" . ($num_str_itogo));


        //
        $activeSheet->mergeCells('A' . ($num + 5) . ':F' . ($num + 5));
        //Высота строки
...
$activeSheet->setCellValue("F" . ($num + 6), "=SUM(F5:F" . ($num + 5) . ")");
...
$activeSheet->setCellValue("E" . ($num + 6), 'Итого:');
...
$activeSheet->getStyle('A4:F4')->getFont()->setName('Malgun Gothic');  /// 'Candara');
$activeSheet->getStyle('A4:F4')->getFont()->setBold(true);
$activeSheet->getStyle('A4:F4')->getFont()->setSize(11);


//$activeSheet->getStyle('A'.($num+4).':J'.($num+4))->getFont()->setBold(true);


$activeSheet->getStyle('A4:F4')->getAlignment()->setHorizontal('center');
$activeSheet->getStyle('A4:A' . ($num + 4))->getAlignment()->setHorizontal('center');
$activeSheet->getStyle('D4:F' . ($num + 7))->getAlignment()->setHorizontal('center');
...
// Голова таблицы
$activeSheet->getStyle('E5:F' . ($num + 4))->getNumberFormat()->setFormatCode(PHPExcel_Style_NumberFormat::FORMAT_TEXT);
$activeSheet->getStyle('E5:F' . ($num + 4))->getAlignment()->setHorizontal('center');

// Тело таблицы
$activeSheet->getStyle('A4:F4')->applyFromArray($styleThinBlackBorderOutline);
$activeSheet->getStyle('A5:F' . ($num + 4))->applyFromArray($styleThinBlackBorderOutline);


//Перенос текста по строкам внутри ячейки
$activeSheet->getStyle('C5:F' . ($num + 4))->getAlignment()->setWrapText(true);
...
foreach ($array_itogo as $item) {
    // ddd($item);


    ///  'id' => 1
    //        's' => 118
    //        'name_gr' => 'Оборудование АСУОП'
    //        'name_el' => 'Помощник водителя GJ-DA04'

    $activeSheet->setCellValue('C' . ($num + 5), $item['name_el']);
    $activeSheet->setCellValue('D' . ($num + 5), $item['s']);

    $num++;
}

$activeSheet->setCellValue("D" . ($num + 6), "=SUM(D" . ($sum_start + 5) . ":D" . ($num + 5) . ")");
$activeSheet->setCellValue('C' . ($num + 6), 'Итого:');
