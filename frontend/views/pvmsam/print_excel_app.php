...
$activeSheet->setCellValue('F7', 'АКТ установки и приема ');
$activeSheet->setCellValue('F8', 'на хранение оборудования (АПП)');
...
$activeSheet->getRowDimension('12')->setRowHeight(110);
...
$activeSheet->setCellValue('A'.$num, 'ПЕРЕДАЛ: Директор ТОО «Транспортный холдинг города Алматы»');
...
$activeSheet->setCellValue('A'.$num, 'ИСПОЛНИТЕЛЬ: Директор ТОО «Транспортная карта»');
$activeSheet->getStyle('A'.$num)->getAlignment()->setWrapText(true);
$activeSheet->mergeCells('A'.$num.':E'.$num);
$activeSheet->getStyle('A'.$num.':E'.$num)->getAlignment()->setHorizontal('justify');
$activeSheet->getRowDimension($num)->setRowHeight(41);
...
/////////////////
        ...
        $file=$_SERVER['DOCUMENT_ROOT'].'/reports/pvmotion/app/'.$filename ;
