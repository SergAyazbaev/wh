...
$activeSheet->setCellValue('A2', 'ЦС. Инвентаризация. Промежуточные остатки.');


//$activeSheet->setCellValue('G1', '1');

//$activeSheet->getStyle('A1:J2')->getAlignment()->setWrapText(true);
$activeSheet->getRowDimension('A2:I2')->setRowHeight(50);
...
$activeSheet->setCellValue('E4', 'Bar_code');
$activeSheet->setCellValue('F4', 'Вх.сальдо');
$activeSheet->setCellValue('G4', 'Монтаж');
$activeSheet->setCellValue('H4', 'Демонтаж');
$activeSheet->setCellValue('I4', 'Исх.сальдо');
...
foreach ($model['array_tk_amort'] as $key_group => $item) {
    ...
    $activeSheet->setCellValue('E' . ($num + 5), "" . (string)$item['bar_code']);
    $activeSheet->setCellValue('F' . ($num + 5), "" . $item['ed_izmer_num']);
    $activeSheet->setCellValue('G' . ($num + 5), "" . $item['prihod_num']);
    $activeSheet->setCellValue('H' . ($num + 5), "" . $item['rashod_num']);
    $activeSheet->setCellValue('I' . ($num + 5), "" . $item['itog']);
