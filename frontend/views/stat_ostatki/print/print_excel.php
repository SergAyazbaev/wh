...
$activeSheet->setCellValue('A2', 'Отчет по количеству накладных на одно устройство за весь период');
...
$activeSheet->setCellValue('E4', 'Сумма повторений');
...
foreach ($model_sklad as $key_group=>$group ) {

    foreach ($group as $key_element =>$element ) {

        foreach ($element as $key_barcode => $barcode) {

        $activeSheet->setCellValue('A' . ($num + 5),  $num  );
        $activeSheet->setCellValue('B' . ($num + 5),  "".$group_name[$key_group] );
        $activeSheet->setCellValue('C' . ($num + 5),  "".$element_name[$key_element] );
        $activeSheet->setCellValue('D' . ($num + 5),  $key_barcode );
        $activeSheet->setCellValue('E' . ($num + 5),  "".$barcode['sum_id'] );
...
$activeSheet->setSharedStyle($sharedStyle2, "A4:F".($num+4));


$activeSheet->getStyle('A3')->getFont()->setBold(true);

$activeSheet->getStyle('A4:F4')->getFont()->setName( 'Malgun' );  /// 'Candara');
