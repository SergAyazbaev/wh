...
if (isset($model->sklad_vid_oper) && ((int)$model->sklad_vid_oper == 2 || (int)$model->sklad_vid_oper == 3)) {
    $this->title = 'Admin ' . ' № ' . $model->id; // Замена
}
...
<div id="tz1">
    ...
    <?php Pjax::begin(['id' => 'pjax-sklad_form']); ?>
...
//                            'pattern' => '[0-9]{5,25}',
//                            'pattern' => '/\d+[A-Z]?[\d{1,5}][-]?[\d{,2}]/',

                            'pattern' => '[\d]+[A-Z]?[\d]{1,5}[\-]?[\d]?[\d]?',
...
if (event.keyCode==13){
        ...
        data = /[^\W]+\d+[A-Z]?[0-9]{1,5}[-]?[\d{,2}]/.exec( data );

//        '/^[0-9]{5,10}[A-Z]?[0-9]{1,5}(-5)?(-8)?(-16)?$/i';
//        alert(data);
        ...
        var bar_code=$(this).val();
                ...
                var otstup_19600 = bar_code.indexOf('19600') ; // 19600002383    // CVB24
                var otstup_28000 = bar_code.indexOf('28000') ; // 28000006582    // CVB44
                var bar_code_obrez = bar_code;

                // CVB24
                if ( otstup_19600 >= 0 ){
                    bar_code=bar_code_obrez.substr( otstup_19600, 11 );
                        //alert(bar_code);
                    $('#code5-{multiple_index_my_id2}').val( bar_code );
                        //alert('otstup ='+otstup);
                }

                // CVB44
                if ( otstup_28000 == 0 ){
                    bar_code=bar_code_obrez.substr( otstup_28000, 11 );
                        //alert(bar_code);
                    $('#code5-{multiple_index_my_id2}').val( bar_code );
                        //alert('otstup ='+otstup);
                }
